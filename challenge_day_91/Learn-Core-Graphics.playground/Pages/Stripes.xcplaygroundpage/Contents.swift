...
let rendered = renderer.image { ctx in
    ...
    // Add your code here
    let colors = [UIColor.orange, UIColor.yellow, UIColor.green, UIColor.blue]
    var colorIndex = 0
    for i in stride(from: 200, to: 1000, by: 200) {
        colors[colorIndex].setFill()
        ctx.cgContext.fill(CGRect(x: i, y: 0, width: 200, height: 1000))
        colorIndex += 1
    }
}
